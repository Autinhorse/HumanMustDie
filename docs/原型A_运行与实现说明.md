...
9. **当前阶段是里程碑 A+（表现层完成度）**，不是里程碑 B —— 见设计文档 §17 的顺序调整说明。
   待办：机关模型与触发动作、打击反馈（命中/击飞/坠落/死亡）、音效、场景收尾。
10. 地块系统（里程碑 B）开工前还需要确认：地块之间的接口对齐规则（是否要求接口宽度一致）、三选一候选位置的挑选策略、以及旋转后不可连接时的标红判定细节。
