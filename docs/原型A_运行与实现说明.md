...
| `under_clouds` | 岛下方的**体积云**（FogVolume）：浓度、边缘虚化、压扁程度、自发光 |
| `core_fx` | 核心的呼吸（发光起伏、水面浮动） |
...
- **环境光遮蔽烘焙进顶点色**，运行时零开销。关键是**地砖要细分**（`board.ao_subdiv`，
  默认 4×4）：一格一个四边形时 AO 只能在四个角取值，暗部会被摊成两米宽的糊，
  几乎看不出效果 —— 第一版就是这么失败的。细分之后按**到墙的距离**衰减
  （`ao_radius`）累加遮蔽，暗部才会贴着墙根柔和铺开。
  `ao_corner` 是强度，`ao_radius` 是暗部往外铺多远（0.55 米 ≈ 四分之一格，给大了会糊成一片），
  `ao_side` 管竖直面底部。另外两条是调的时候踩出来的：
  - **多个遮挡物之间要取最大值，不能累加**。累加的话墙中段会被好几格叠满、端头只有一格，
    亮度差很大，墙的两端就会出现明显的突变缺口。
  - **缝线层要和面色层用同样的细分**，否则缝线不跟着变暗，会在暗部里留下一张亮网格。
...
- **坑不要补底板**（`under_clouds`）。一开始的做法是给坑底垫一块暗面，结果很假。
  参考图的逻辑是：**岩体一直往下延伸，再往下被云挡住** —— 坑壁本来就是相邻格子的岩体，
  所以只需要在岛下方铺一层云，坑里和岛缘看到的都是它，不用为坑做任何特殊处理。
  岩体也相应加长（`rock.solid_depth` / `edge_depth_max`），让它能扎进云层里。
...
- **描边必须随拉远淡出**（`environment.outline_fade_start` / `outline_fade_end`）：
  屏幕空间描边的线宽是固定像素，单位拉远后只有二十来像素高，两边各夹一条线就把主体
  吃掉了，看起来是一团黑糊。拉远时交给地面标记，拉近才靠线。
- 描边用**乘算压暗**（`blend_mul`）而不是盖一层黑：乘算保留底下的色相，远看是深色
  边缘而不是黑边。`outline_color` 是乘算系数，越小越黑。
...
```
spring  弹簧板  地面，弹出侧边框里有凹槽 + 发光条 + 两端铆钉，板面箭头平嵌
spikes  尖刺板  地面，浅灰板开四个方槽，槽里各一根刺
push    推板    墙面，宽高比 2:1，中间浅灰面板带宝石
```

**活动件和固定件分开**：每个模型都是 `Frame`（石缘、金框、铆钉、发光条）
+ `Mover`（刺 / 弹簧板 / 推板本体）。三种统一叫 `Mover`，`traps.json` 的
`anim.part` 不用按类型分别配。

`traps.json` 里 `model` + `tier` 组成文件名（`board_spikes` + tier 2 →
`board_spikes_2`）。这个拼接只能有一处，写在 `TrapView.model_for()` 里 ——
游戏和测试各算各的就会出岔子（T7 抓到过一次）。

**静止姿态就是导出的姿态**：刺尖齐板面、箭头平嵌在板里、推板齐金框正面，
所以 `anim.rest` 全是 0，`fire` 才是弹出量。这样"有没有平齐"直接看导出的
模型就能判断，不用在游戏里一帧帧找。

### 配色必须按"上屏值"反推（v0.0.35）

**贴图色不等于屏幕上的颜色。** 游戏场景的环境光很亮，实测存在一个亮度地板：

```
屏幕值 ≈ 0.31 + 0.71 × 贴图色      （在游戏环境里取两点拟合）
```

所以按设计图直接吸色填进去是不对的 —— 设计图槽色 0.23 填进去，上屏是 0.475，
一点都不暗，整块板糊成一片。现在 `COLORS` 里写的是**反推**出来的贴图色：
目标屏幕值代入上式求解。改环境光（ambient_energy / 色调映射）以后这条拟合
要重测。

顺带记一下：SSAO 的 radius 是 1.0 米，而槽深只有 0.12 米，指望 SSAO 把槽压暗
是不行的 —— 槽的深色只能自己烘进材质，槽壁还得单独贴一圈暗色内衬，
否则槽壁用的是浅灰内板本身，只有槽底暗，深度读不出来。

判断"像不像"要用数值，不能靠看：把游戏截图和设计图同一块板的亮度分位数
（2% / 10% / 中位 / 90%）并排打出来，比肉眼可靠得多。

### 这一轮踩到的
- **整体发灰发白**：Blender 的 Base Color 收的是**线性**值，从参考图上吸到的
  sRGB 数值不能直接填。加了 `to_linear()`。
- **内板做成了接近黑**：参考图里内板只比地砖略灰（实测 0.845/0.815/0.781），
  槽底也只是中灰（0.40/0.43/0.44），第一版当成暗色槽做黑了，差得很远。
- **金在游戏里变成暗褐色**：Blender 里 metallic 0.85 的金很漂亮，但 Godot 这边
  只有环境色、没有反射探针，金属度高的面直接发黑。降到 0.30 两边都是金色。
- **尖刺大了一倍还溢出边框**：算槽中心时误用了槽的外沿而不是中心
  （`d = div + hole` 应为 `div + hole_w * 0.5`）。
- **墙面板渲染出一排空框**：板子朝 +Y，预览相机却在 -Y 那侧，拍到的是背面。
- **宝石在墙面板上是一条缝**：轮廓建在 XY 平面、尖朝 +Z。`prism`/`gem` 加了
  `axis` 参数，墙面板的轮廓直接建在 XZ 平面沿 Y 挤出 —— 建完再转 90° 位移
  极难对齐。
...
8. **当前阶段是里程碑 A+（表现层完成度）**，不是里程碑 B —— 见设计文档 §17 的顺序调整说明。

   已完成：
   - 机关模型与触发动作（§6.8）、打击反馈（§6.9，命中/击飞/落地/死亡/坠坑）
   - 三种机关面板（尖刺 / 弹簧 / 推板）× 三个等级，照 `ref/3 boards.png` 做，
     已接进游戏并逐部件打磨过一轮 —— 见 [`机关面板_部件说明.md`](机关面板_部件说明.md)
   - 顶点色 AO + 棱高光，每次构建自动重烘

   还没做：
   - **面板的等级 2/3 目前只是换色**，花纹没按等级做区分
   - **黏胶地面和旋转锯墙还是最早的机械造型**（`build_traps.py`），没进面板体系
   - 音效
   - 场景收尾（体积云的颜色/范围/浓淡还可以慢慢调）
